...
<x-layout bodyClass="g-sidenav-show  bg-gray-200">
    ...
    <main class="main-content position-relative max-height-vh-100 h-100 border-radius-lg ">
        ...
        <!-- End Navbar -->
        @if (isset($people))
            <form method="post" action='{{ url('/edit-branch-manager/' . $people->id) }}'>
            @else
                <form method="post" action='{{ url('/add-branch-manager') }}'>
        @endif
        @csrf
                    ...
                    <div class="input-group input-group-outline my-2">
                        <select class="form-control" name='branch_id'>
                            ...
                            @foreach ($branches as $branch)
                                <option value="{{ $branch->id }}" @if (isset($people) && $branch->id == $people->branch_id) selected @endif>
                                    {{ $branch->name }}
                                </option>
                            @endforeach
                        </select>
